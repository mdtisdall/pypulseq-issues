# Compute the SAFE low-pass filter in calculate_pns as a recursion
...
sequence (a trapezoid on each axis every 10 ms), and prints the times:
...
I am happy to open a PR with this change and a test that compares the recursion with
the convolution, for each time constant of `safe_example_hw()` and in `safe_gwf_to_pns`
on `safe_example_gwf()`. Are there any concerns about the suggested change, or about
where it should go while #385 is open?
